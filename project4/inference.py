...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        # this for loop will perform inference by interleaving joining on a variable and eliminating it....
        for el_variable in eliminationOrder:
            unjoined_factors, joined_factors = joinFactorsByVariable(factors, el_variable)
            factors = unjoined_factors
            
            if len(joined_factors.unconditionedVariables()) != 1:
                factors.append(eliminate(joined_factors, el_variable))
