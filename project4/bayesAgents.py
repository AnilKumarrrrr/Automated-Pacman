...
def constructBayesNet(gameState):
    ...
    for house in gameState.getPossibleHouses():
        #by default function given in above commented part
        for walls in gameState.getHouseWalls(house):
            obsVars.append(OBS_VAR_TEMPLATE % walls)
            obs = OBS_VAR_TEMPLATE % walls
            
            #define all edges tuplle
            edges.append((FOOD_HOUSE_VAR, obs))    #tuple edges of food house
            edges.append((GHOST_HOUSE_VAR, obs))   #tuple edges of ghost house
            
             #add positions and values into variableDomainsDict for observation variable
            variableDomainsDict[OBS_VAR_TEMPLATE % walls] = [BLUE_OBS_VAL, RED_OBS_VAL, NO_OBS_VAL]
            
    #making edges between  x,y and food house
    edges.append((X_POS_VAR, FOOD_HOUSE_VAR))
    edges.append((Y_POS_VAR, FOOD_HOUSE_VAR))
    
    #making edges between  x,y and ghost house
    edges.append((X_POS_VAR, GHOST_HOUSE_VAR))
    edges.append((Y_POS_VAR, GHOST_HOUSE_VAR))

    #add positions and values into variableDomainsDict for food and ghost
    ...
    #add positions and values into variableDomainsDict for x and y variables
...
def fillYCPT(bayesNet, gameState):
    ...
    "*** YOUR CODE HERE ***"
    #difine values and prob of all four sides
    #the prior probability over the y position variable
    #probability for top
    yFactor.setProbability({Y_POS_VAR: BOTH_TOP_VAL}, PROB_BOTH_TOP)   
    #probability for bottom
    yFactor.setProbability({Y_POS_VAR: BOTH_BOTTOM_VAL}, PROB_BOTH_BOTTOM)
    #probability for left top
    yFactor.setProbability({Y_POS_VAR: LEFT_TOP_VAL}, PROB_ONLY_LEFT_TOP)
    #probability for left bottom
